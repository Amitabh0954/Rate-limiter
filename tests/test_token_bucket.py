...
def test_allows_burst_up_to_capacity(r):
    now, _ = make_clock(0)
    for i in range(5):
        allowed, tokens = is_allowed(r, "clientA", capacity=5, refill_rate=1, now_fn=now)
...
def test_blocks_once_bucket_is_empty(r):
    ...
    allowed, tokens = is_allowed(r, "clientB", capacity=5, refill_rate=1, now_fn=now)
...
def test_tokens_refill_over_time(r):
    ...
    allowed, tokens = is_allowed(r, "clientC", capacity=5, refill_rate=1, now_fn=now)
...
def test_refill_is_capped_at_capacity(r):
    ...
    allowed, tokens = is_allowed(r, "clientD", capacity=5, refill_rate=1, now_fn=now)
...
def test_clients_have_independent_buckets(r):
    ...
    allowed, tokens = is_allowed(r, "clientF", capacity=5, refill_rate=1, now_fn=now)
...
def test_fractional_refill_across_small_time_slices(r):
    ...
    allowed, tokens = is_allowed(r, "clientG", capacity=10, refill_rate=2, now_fn=now)
